  ...
  <div class="relative min-h-screen bg-white flow-root">
    ...
    <div class="relative mt-[156px] mx-[146px] flex flex-col gap-6 pb-10 max-[1024px]:mx-6 max-[1024px]:mt-[140px]">
      ...
      <div class="bg-white border-l-4 border-[#005b31] rounded-xl shadow-[0_1px_1px_rgba(0,0,0,0.05)] pl-7 pr-6 py-6 flex flex-row justify-between items-center gap-6 max-w-[640px] max-[640px]:max-w-none">
        ...
        @if($student->pas_foto)
          <div class="shrink-0">
            <img src="{{ asset($student->pas_foto) }}" class="w-[90px] h-[120px] object-cover rounded-lg border border-gray-300 shadow-sm" alt="Pas Foto Calon Murid">
          </div>
        @else
      ...
      <form action="{{ route('panitia.grading', $student->id_murid) }}" method="POST" class="bg-white border border-[#becabe] rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.05)] overflow-hidden max-w-[800px] w-full mx-auto">
        ...
        <div class="p-6 flex flex-col gap-6">
          ...
          <div class="flex flex-col gap-1">
            <label for="nilai_hafalan" class="text-[#121c2a] text-[14px] font-semibold tracking-[0.7px]" style="font-family: 'WorkSans-SemiBold', sans-serif;">1. Nilai Hafalan (1 - 10)</label>
            <input type="number" name="nilai_hafalan" id="nilai_hafalan" required min="1" max="10"
                   value="{{ $student->hasil->nilai_hafalan ?? '' }}" placeholder="Masukkan nilai hafalan (1 - 10)"
          ...
          <!-- 2. Nilai Wawancara -->
          <div class="flex flex-col gap-1">
            <label for="nilai_wawancara" class="text-[#121c2a] text-[14px] font-semibold tracking-[0.7px]" style="font-family: 'WorkSans-SemiBold', sans-serif;">2. Nilai Wawancara (1 - 10)</label>
            <input type="number" name="nilai_wawancara" id="nilai_wawancara" required min="1" max="10"
                   value="{{ $student->hasil->nilai_wawancara ?? '' }}" placeholder="Masukkan nilai wawancara (1 - 10)"
                   class="w-full px-[17px] py-3 text-[14px] text-gray-800 border border-[#becabe] rounded-lg bg-[#f8f9ff] outline-none focus:border-[#298752]">
          </div>

          <!-- 3. Nilai Calistung -->
          <div class="flex flex-col gap-1">
            <label for="nilai_calistung" class="text-[#121c2a] text-[14px] font-semibold tracking-[0.7px]" style="font-family: 'WorkSans-SemiBold', sans-serif;">3. Nilai Calistung (1 - 10)</label>
            <input type="number" name="nilai_calistung" id="nilai_calistung" required min="1" max="10"
                   value="{{ $student->hasil->nilai_calistung ?? '' }}" placeholder="Masukkan nilai calistung (1 - 10)"
                   class="w-full px-[17px] py-3 text-[14px] text-gray-800 border border-[#becabe] rounded-lg bg-[#f8f9ff] outline-none focus:border-[#298752]">
          </div>

          <!-- 4. Nilai Tasmi -->
          <div class="flex flex-col gap-1">
            <label for="nilai_tasmi" class="text-[#121c2a] text-[14px] font-semibold tracking-[0.7px]" style="font-family: 'WorkSans-SemiBold', sans-serif;">4. Nilai Tasmi (1 - 10)</label>
            <input type="number" name="nilai_tasmi" id="nilai_tasmi" required min="1" max="10"
                   value="{{ $student->hasil->nilai_tasmi ?? '' }}" placeholder="Masukkan nilai tasmi (1 - 10)"
                   class="w-full px-[17px] py-3 text-[14px] text-gray-800 border border-[#becabe] rounded-lg bg-[#f8f9ff] outline-none focus:border-[#298752]">
          </div>

          <!-- 5. Nilai Mandiri -->
          <div class="flex flex-col gap-1">
            <label for="nilai_mandiri" class="text-[#121c2a] text-[14px] font-semibold tracking-[0.7px]" style="font-family: 'WorkSans-SemiBold', sans-serif;">5. Nilai Mandiri (1 - 10)</label>
            <input type="number" name="nilai_mandiri" id="nilai_mandiri" required min="1" max="10"
                   value="{{ $student->hasil->nilai_mandiri ?? '' }}" placeholder="Masukkan nilai mandiri (1 - 10)"
                   class="w-full px-[17px] py-3 text-[14px] text-gray-800 border border-[#becabe] rounded-lg bg-[#f8f9ff] outline-none focus:border-[#298752]">
          </div>

          <!-- 6. Jalur Prestasi -->
          <div class="border border-[#becabe] rounded-lg p-[17px] flex flex-col gap-2">
            <div class="flex items-center gap-3">
              <input type="checkbox" id="prestasi_check"
                     class="w-5 h-5 cursor-pointer rounded border border-gray-300"
                     {{ $student->piagram_kejuaraan ? 'checked' : '' }} disabled>
            ...
            @if($student->piagram_kejuaraan)
              <div class="mt-2 text-[13px] text-gray-600 pl-8" style="font-family: 'WorkSans-Regular', sans-serif;">
                <p class="mt-2">
                  <a href="{{ route('document.preview', ['type' => 'pas_foto', 'filename' => basename($student->piagram_kejuaraan)]) }}" target="_blank" class="text-blue-600 hover:text-blue-800 font-bold underline" style="display: inline-flex; align-items: center; gap: 4px;">
                    🔍 Cek Berkas Piagam Kejuaraan
                  </a>
                </p>
              </div>
            @endif
